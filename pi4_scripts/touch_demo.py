...
if on_pitft():
    import sys
    sys.path.append('/home/qz425/final/pigame')
    sys.path.append('/home/qz425/final/pitft_touchscreen')
    import pigame
...
# 初始化音频混音器（使用较小的buffer以减少延迟，增加通道数以支持并发音效）
try:
    if on_pitft():
        # Pi需要特定的音频设置 - 使用较小的buffer以提高响应速度
        pygame.mixer.init(frequency=44100, size=-16, channels=2, buffer=256)
        pygame.mixer.set_num_channels(32)  # 增加混音通道数到32，确保足够的并发音效
        print("✓ 音频混音器已初始化 (Pi模式, 32通道, 256字节缓冲)")
    else:
...
# 根据是否在PiTFT上运行调整字体大小
if on_pitft():
    # PiTFT 320x240 分辨率使用较小字体
    font_small = load_font_with_fallback(8)   # 原14 -> 8
    font_medium = load_font_with_fallback(12)  # 原20 -> 12
    font_big = load_font_with_fallback(28)     # 原58 -> 28
else:
    # PC 800x600 分辨率使用标准字体
    font_small = load_font_with_fallback(14)
    font_medium = load_font_with_fallback(20)
...
class GameWorld:
    def __init__(self, scale='large'):
        ...
        if LLM_AI_AVAILABLE and hasattr(ai_config, 'USE_LLM_AI') and ai_config.USE_LLM_AI:
            try:
                use_ollama = getattr(ai_config, 'USE_OLLAMA', False)
                
                if use_ollama:
                    # 使用免费的Ollama本地AI
                    ollama_model = getattr(ai_config, 'OLLAMA_MODEL', 'qwen2.5:3b')
                    difficulty = getattr(ai_config, 'AI_DIFFICULTY', 'normal')
                    self.llm_ai = LLMAIPlayer(use_ollama=True, ollama_model=ollama_model, difficulty=difficulty)
                    ...
                    if api_key and api_key != 'sk-your-api-key-here':
                        self.llm_ai = LLMAIPlayer(api_key=api_key, model=model, difficulty=difficulty)
                        if self.llm_ai.is_available():
                            self.use_llm_ai = True
                            print("✓ 使用LLM AI (GPT-4)")
                        else:
                            print("⚠ LLM AI初始化失败，使用规则AI")
                    else:
                        print("⚠ 未配置有效的API密钥，使用规则AI")
            except Exception as e:
        ...
        if elapsed >= body.siege_duration:
            ...
            if body.defender_count == 0:
                ...
                if winner == 'attacker':
                    ...
                    print(f"战斗胜利: {body.name}, 剩余 {len(survivors)} 艘舰船")
                elif winner == 'defender':
                    # 防守方胜利
                    body.fleet_count = len(survivors)
                    body.units = survivors
                    print(f"战斗失败: {body.name}, 敌方剩余 {len(survivors)} 艘舰船")
                else:
                    # 同归于尽
                    body.owner = None
                    body.fleet_count = 0
    ...
    def draw(self, surface):
        ...
        pause_btn_x = 220
        pause_btn_y = 8
        pause_btn_rect = pygame.Rect(pause_btn_x, pause_btn_y, pause_btn_w, pause_btn_h)
        pause_btn_color = (100, 100, 200) if not self.paused else (200, 100, 100)
        pygame.draw.rect(surface, pause_btn_color, pause_btn_rect)
        pygame.draw.rect(surface, WHITE, pause_btn_rect, 2)
        pause_text = "Resume" if self.paused else "Pause"
        surface.blit(font_small.render(pause_text, True, WHITE), (pause_btn_x + 25, pause_btn_y + 8))
        self._pause_btn_rect = pause_btn_rect
        
        # Draw "Buy Worker" button (costs 5 food)
        buy_worker_btn_w = 180  # 缩短宽度从200到180
        buy_worker_btn_h = 34
        buy_worker_btn_x = pause_btn_x + pause_btn_w + 10  # 在暂停按钮右侧
        buy_worker_btn_y = 8
        ...
        buy_worker_text = "Buy Worker (5F)"  # 缩短文字
        ...
        exit_btn_w = 70
        exit_btn_h = 34
        exit_btn_x = W - 80  # 从右边开始
        exit_btn_y = 8
        exit_btn_rect = pygame.Rect(exit_btn_x, exit_btn_y, exit_btn_w, exit_btn_h)
        pygame.draw.rect(surface, (180, 60, 60), exit_btn_rect)
        pygame.draw.rect(surface, WHITE, exit_btn_rect, 2)
        surface.blit(font_small.render('Exit', True, WHITE), (exit_btn_x + 18, exit_btn_y + 8))
        self._quit_btn_rect = exit_btn_rect
        
        # Draw "Ores" button (to the left of Exit)
        ores_btn_w = 70
        ores_btn_h = 34
        ores_btn_x = exit_btn_x - ores_btn_w - 8  # Exit左边
        ores_btn_y = 8
        ores_btn_rect = pygame.Rect(ores_btn_x, ores_btn_y, ores_btn_w, ores_btn_h)
        pygame.draw.rect(surface, (60, 60, 90), ores_btn_rect)
        pygame.draw.rect(surface, WHITE, ores_btn_rect, 2)
        surface.blit(font_small.render('Ores', True, WHITE), (ores_btn_x + 16, ores_btn_y + 8))
        self._ores_button_rect = ores_btn_rect
        
        # Draw "Trade" button (to the left of Ores)
        trade_btn_w = 70
        trade_btn_h = 34
        trade_btn_x = ores_btn_x - trade_btn_w - 8  # Ores左边
        trade_btn_y = 8
        ...
        pygame.draw.rect(surface, WHITE, trade_btn_rect, 2)
        surface.blit(font_small.render('Trade', True, WHITE), (trade_btn_x + 12, trade_btn_y + 8))
        self._trade_button_rect = trade_btn_rect
        
        # Draw "Units" button (to the left of Trade)
        units_btn_w = 70
        units_btn_h = 34
        units_btn_x = trade_btn_x - units_btn_w - 8  # Trade左边
        units_btn_y = 8
        ...
        pygame.draw.rect(surface, WHITE, units_btn_rect, 2)
        surface.blit(font_small.render('Units', True, WHITE), (units_btn_x + 14, units_btn_y + 8))
        ...
        if getattr(self, 'show_trade_popup', False):
            ...
            if self.trade_view == 'sell':
                ...
                popup_surf.blit(font_small.render(f"Workers (unassigned): {int(self.workers)}  Sell ${worker_sell_price:.2f}", True, BLACK), (10, y))
        ...
        if getattr(self, 'show_unit_popup', False):
            ...
            # 标题显示当前页码
            title = f'Warrior Units (Page {current_page + 1}/{total_pages})'
            popup_surf.blit(font_medium.render(title, True, BLACK), (10, 10))
            ...
            # 上一页/下一页按钮
            prev_rect = pygame.Rect(10, popup_h - 40, 80, 30)
            next_rect = pygame.Rect(popup_w - 90, popup_h - 40, 80, 30)
            ...
            for unit_name, unit_data, category in page_units:
                ...
                if special == 'anti_starbot':
                    special_text = "Special: +100% vs Starbot"
                    popup_surf.blit(font_small.render(special_text, True, (200, 50, 50)), (10, y))
                    y += 18
                elif special == 'convert':
                    special_text = "Special: Convert enemy"
                    popup_surf.blit(font_small.render(special_text, True, (50, 100, 200)), (10, y))
                    y += 18
                else:
                    y += 5
                ...
                if len(price_parts) > 3:
                    ...
                    y -= 18  # Move back up for button positioning
                else:
        ...
        if self.game_over:
            ...
            # 提示按ESC返回菜单
            hint_surf = font_small.render("按 ESC 返回菜单", True, (200, 200, 200))
        ...
        if getattr(self, 'show_unit_select_popup', False):
            ...
            if hasattr(self, '_unit_select_confirm_button') and self._unit_select_confirm_button.collidepoint(x, y):
                if self._unit_select_can_confirm:
                    ...
                    self.show_unit_select_popup = False
                    self.selected_units_to_send = []
                    print(f"派遣 {qty} 艘舰船携带 {len(units_to_send)} 个单位到 {dest.name}")
                    return True
        ...
        if getattr(self, 'show_unit_popup', False):
            ...
            for btn in getattr(self, '_unit_buy_buttons', []):
                if btn['rect'].collidepoint(x, y) and btn.get('can_afford', False):
                    ...
                    print(f"在 {target_planet.name} 购买了 {unit['name']}")
    ...
    def ai_make_decision(self):
        ...
        if self.use_llm_ai and self.llm_ai:
            ...
            # 创建后台线程，daemon=True确保不阻塞程序退出
            thread = threading.Thread(
                target=self._llm_ai_thread_worker, 
                daemon=True,
                name="LLM-AI-Worker"
            )
            thread.start()
    ...
    def _llm_ai_thread_worker(self):
        """LLM AI后台线程工作函数（异步执行，不阻塞主线程和音频）"""
        import time
        try:
            print("🤖 AI正在后台思考中...")
            start_time = time.time()
            
            # 在独立线程中执行API调用，网络 I/O会自动释放GIL
            ...
            if action:
                print(f"✓ AI决策完成，耗时 {elapsed:.1f}秒")
            else:
                print(f"✓ AI选择等待，耗时 {elapsed:.1f}秒")
        except Exception as e:
            print(f"✗ LLM AI决策失败: {e}，将使用规则AI")
            import traceback
            traceback.print_exc()
            with self.ai_thread_lock:
                self.ai_pending_action = None
...
difficulty_buttons = {'Easy (简单)': (W//2, H//2 - 60), 'Normal (普通)': (W//2, H//2), 'Hard (困难)': (W//2, H//2 + 60), 'Back': (100, 50)}
...
def main():
    ...
    while running:
        ...
        if game_state == GameState.MENU:
            ...
            # 显示难度选择标题
            title_text = font_big.render("Select AI Difficulty", True, WHITE)
            lcd.blit(title_text, title_text.get_rect(center=(W//2, 150)))
